...
def reset_active_tracks(active_tracks, current_frame):
    for track_id in list(active_tracks.keys()):
        # If a track is not in the current frame and hasn't been marked as lost yet
        time_alive = active_tracks[track_id]["last_seen"] - active_tracks[track_id]["first_seen"]
        print(f"################# TRACKING EVENT - LOST ITEM {track_id}  ##################")
        print(f"Track {track_id} ({active_tracks[track_id]['class_name']}) lost at frame {current_frame}. Was alive for {time_alive} frames.")
        del active_tracks[track_id] 



def run_tacking_update(boxes, active_tracks, class_names, current_frame):
    ...
    for track_id, info in current_tracks_info.items():
        if track_id not in active_tracks:
            ...
            print(f"################# TRACKING EVENT - NEW ITEM {track_id} ##################")
            print(f"New item: {active_tracks[track_id]}")
        else:
            ...
            if active_tracks[track_id]["class_id"] != info["class_id"]:
                print(f"Warning: Track {track_id} changed class from {active_tracks[track_id]['class_name']} to {info['class_name']} at frame {current_frame}")
    ...
    for track_id in list(active_tracks.keys()):
        ...
        if track_id not in current_track_ids:
            time_alive = active_tracks[track_id]["last_seen"] - active_tracks[track_id]["first_seen"]
            print("################# TRACKING EVENT - LOST ITEM  ##################")
            print(f"Track {track_id} ({active_tracks[track_id]['class_name']}) lost at frame {current_frame}. Was alive for {time_alive} frames.")
            del active_tracks[track_id] 
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_weights", default='./runs/detect/train7/weights/best.pt')
    ...
    while cap.isOpened(): 
        ...
        results = model.track(frame, conf=args.confidence_level, persist=True, tracker="bytetrack.yaml", verbose=False)

         # Get the current tracks
        if results[0].boxes.id is not None:
            boxes = results[0].boxes
            run_tacking_update(boxes, active_tracks, class_names, frame_count)
        else:
            if bool(active_tracks):
                reset_active_tracks(active_tracks, frame_count)
        frame_count += 1
