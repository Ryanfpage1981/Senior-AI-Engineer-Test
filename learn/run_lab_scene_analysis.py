...
def reset_active_tracks(active_tracks, current_frame, tracking_file):
    for track_id in list(active_tracks.keys()):
        ...
        tracking_file.write(f"################# TRACKING EVENT - LOST ITEM {track_id}  ##################\n")
        tracking_file.write(f"Track {track_id} ({active_tracks[track_id]['class_name']}) lost at frame {current_frame}. Was alive for {time_alive} frames.\n")
...
def run_tacking_update(boxes, active_tracks, class_names, current_frame, tracking_file):
    ...
    for track_id, info in current_tracks_info.items():
        if track_id not in active_tracks:
            ...
            tracking_file.write(f"################# TRACKING EVENT - NEW ITEM {track_id} ##################\n")
    ...
    for track_id in list(active_tracks.keys()):
        ...
        if track_id not in current_track_ids:
            time_alive = active_tracks[track_id]["last_seen"] - active_tracks[track_id]["first_seen"]
            tracking_file.write("################# TRACKING EVENT - LOST ITEM  ##################\n")
            tracking_file.write(f"Track {track_id} ({active_tracks[track_id]['class_name']}) lost at frame {current_frame}. Was alive for {time_alive} frames.\n")
            del active_tracks[track_id] 
...
if __name__ == '__main__':
    ...
    while cap.isOpened(): 
        ...
        if results[0].boxes.id is not None:
            ...
            run_tacking_update(boxes, active_tracks, class_names, frame_count, tracking_evt_log)
        else:
            if bool(active_tracks):
                reset_active_tracks(active_tracks, frame_count, tracking_evt_log) ##incase we have no tracks
        frame_count += 1
