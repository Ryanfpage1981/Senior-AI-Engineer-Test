...
def generate_hand_dataset(class_data, class_imgs_path, class_id):
    ...
    hand_files = [
        f
        for f in os.listdir(class_imgs_path)
        if os.path.isfile(os.path.join(class_imgs_path, f))
    ]
    ...
    for i, j in zip(hand_files, class_data):
        ...
        img_label = (
            []
        )  ## Make consistent with other classes that can have more than one instance per image
...
def generate_syn_dataset(
    class_data, class_imgs_path, n_samples_in_img, class_id, bkg_img
):
    """
    ...
    roi_files = [
        os.path.join(class_imgs_path, f) for f in roi_files
    ]  ##add path back ##TODO redo data storage
    step_size_x = (
        class_data[0][2][2] - class_data[0][2][0] // 2
    )  ##approx image x of box ##TODO better tracking of size and placement
    step_size_y = (
        class_data[0][2][3] - class_data[0][2][1] // 2
    )  ##approx image y of box ##TODO better tracking of size and placement
    n_labels = 0
    for i in n_samples_in_img:
        ...
        if i > 0:
            ...
            img_labels = []
            for j in range(i):
                epsilon = np.random.randint(
                    0, 50
                )  ##Random jitter from original location
                if num_original_samples > 1:
                    sample_idx = np.random.randint(
                        0, num_original_samples - 1
                    )  ##Sample the examples of the class we have TODO Sample better
                print(f"Sample {sample_idx}")
                orig_pos = class_data[sample_idx][2]
                random_step_x = np.random.randint(
                    0, step_size_x
                )  ##TODO Experiement with +/- shiting
                random_step_y = np.random.randint(0, step_size_y)
                new_centre = [
                    orig_pos[2] - orig_pos[0] // 2 + epsilon + random_step_x * j,
                    orig_pos[3] - orig_pos[1] // 2 + epsilon + random_step_y * j,
                ]
                print(f"Oringal pos {orig_pos}")
                try:
                    img, bbox = data_augmentation.compositor(
                        img, cv2.imread(roi_files[sample_idx]), new_centre
                    )
                    img_labels.append([class_id, bbox])
                    n_labels += 1
                except Exception as e:
                    print(
                        "Error generating image ignore this sample"
                    )  ##This happens if we go out of bounds currently need to fix in the compositor
                    pass
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--bkg_image", default="data/original/bkg_images/lab_scene_bkg_0.png"
    )
    parser.add_argument(
        "--cvat_file", default="data/cvat_annotations/labels_run_0_2000.xml"
    )
    parser.add_argument("--class_data_image_path", default="data/original")
    parser.add_argument("--n_training_images", default=60)
    parser.add_argument(
        "--n_class_labels", default=130
    )  ##This is going to create an unbalanced dataset - with hands
    ...
    imgs_h, labels_h, n_labels_h = generate_hand_dataset(
        class_data["Hand"],
        os.path.join(args.class_data_image_path, "Hand"),
        classmapping["Hand"],
    )
    ...
    max_pd_per_image = 5
    min_pd_per_image = 1
    dish_samples_per_image = create_random_array_with_max_sum(
        n_training_images, min_pd_per_image, max_pd_per_image, n_training_labels
    )
    np.random.shuffle(
        dish_samples_per_image
    )  ##shuffle in case we have loads of 0 at the end if we reach max early

    max_b_per_image = 2
    min_b_per_image = 1
    bottle_samples_per_image = create_random_array_with_max_sum(
        n_training_images, min_b_per_image, max_b_per_image, n_training_labels
    )
    np.random.shuffle(bottle_samples_per_image)

    imgs_b, labels_b, n_labels_b = generate_syn_dataset(
        class_data["Bottle"],
        os.path.join(args.class_data_image_path, "Bottle"),
        bottle_samples_per_image,
        classmapping["Bottle"],
        cv2.imread(args.bkg_image),
    )

    imgs_pd, labels_pd, n_labels_pd = generate_syn_dataset(
        class_data["PetriDish"],
        os.path.join(args.class_data_image_path, "PetriDish"),
        dish_samples_per_image,
        classmapping["PetriDish"],
        cv2.imread(args.bkg_image),
    )

    ##split each dataset into train and validation before writing
    ##easy for hands
    h_t, h_v = split_dataset_by_n_labels(
        labels_h, n_labels_h, len(imgs_h), frac_t=args.training_frac
    )
    b_t, b_v = split_dataset_by_n_labels(
        labels_b, n_labels_b, len(imgs_b), frac_t=args.training_frac
    )
    pd_t, pd_v = split_dataset_by_n_labels(
        labels_pd, n_labels_pd, len(imgs_pd), frac_t=args.training_frac
    )
    ...
    data_io.write_labels_to_file(
        combined_ts_labels, ds_loc[1], convert_from_cvat_to_yolo=True
    )
    data_io.write_labels_to_file(
        combined_vs_labels, ds_loc[3], convert_from_cvat_to_yolo=True
    )
