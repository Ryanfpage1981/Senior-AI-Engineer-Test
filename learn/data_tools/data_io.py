...
def write_labels_to_file(
    ...
    output_dir,
    file_prefix="image",
    convert_from_cvat_to_yolo=True,
    base_img_dim=[1280, 720],
):
    """
...
def extract_images_from_video(input_video_file, frame_ids, roi=None):
    ...
    if max(frame_ids) >= total_frames:
        raise ValueError(
            f"Error: Requested frame {max(frame_ids)} exceeds total frames {total_frames}"
        )
                ...
                if int(box.get("occluded")) == 0:
                    ...
                    if class_label not in class_data:
                        class_data[class_label] = [
                            [class_id, frame_id, [xmin, ymin, xmax, ymax]]
                        ]
                    else:
                        class_data[class_label].append(
                            [class_id, frame_id, [xmin, ymin, xmax, ymax]]
                        )
